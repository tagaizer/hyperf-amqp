...
use Tagaizer\HyperfAmqp\Event\AfterConsume;
use Tagaizer\HyperfAmqp\Event\BeforeConsume;
use Tagaizer\HyperfAmqp\Event\FailToConsume;
use Tagaizer\HyperfAmqp\Event\WaitTimeout;
use Tagaizer\HyperfAmqp\Exception\MessageException;
use Tagaizer\HyperfAmqp\Message\ConsumerMessageInterface;
use Tagaizer\HyperfAmqp\Message\MessageInterface;
use Tagaizer\HyperfAmqp\Message\Type;
...
class Consumer extends Builder
{
    ...
    public function declare(MessageInterface $message, ?AMQPChannel $channel = null, bool $release = false): void
    {
        ...
        try {
            ...
            if(!empty($message->getDlxExchange()) || (!empty($message->getDlxExchange()) && !empty($message->getDlxRoutingKey())) ){
                $exBuilder = $message->getExchangeBuilder();
                $channel->exchange_declare($exBuilder->getDlxExchange(),$exBuilder->getType(), $exBuilder->$exBuilder(), $exBuilder->isDurable(), $exBuilder->isAutoDelete(), $exBuilder->isInternal(), $exBuilder->isNowait(), $exBuilder->getArguments(), $exBuilder->getTicket());
                $channel->queue_declare($builder->getDlxQueue(),$builder->isPassive(), $builder->isDurable(), $builder->isExclusive(), $builder->isAutoDelete(), $builder->isNowait(), $builder->getArguments(), $builder->getTicket());
                $dlxRoutineKeys = (array) $message->getDlxRoutingKey();
                foreach ($dlxRoutineKeys as $dlxRoutingKey) {
                    $channel->queue_bind($builder->getDlxQueue(),$exBuilder->getDlxExchange(),$dlxRoutingKey);
                }
            }
