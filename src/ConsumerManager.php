...
use Tagaizer\HyperfAmqp\Annotation\Consumer as ConsumerAnnotation;
...
class ConsumerManager
{
    ...
    public function run()
    {
        ...
        foreach ($classes as $class => $annotation) {
            ...
            $annotation->dlxExchange && $instance->setDlxExchange($annotation->dlxExchange);
            $annotation->dlxRoutingKey && $instance->setDlxRoutingKey($annotation->dlxRoutingKey);
            $annotation->dlxQueue && $instance->setDlxQueue($annotation->dlxQueue);
    ...
    private function createProcess(ConsumerMessageInterface $consumerMessage): AbstractProcess
    {
        return new class($this->container, $consumerMessage) extends AbstractProcess {
            /**
             * @var \Tagaizer\HyperfAmqp\Consumer
             */
            private $consumer;
